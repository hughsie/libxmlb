...
typedef gboolean (*FuUtilPrivateCb)(XbToolPrivate *util, gchar **values, GError **error);
...
static gboolean
xb_tool_run(XbToolPrivate *priv, const gchar *command, gchar **values, GError **error)
{
	/* find command */
	...
	/* not found */
	g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_FAILED, "Command not found");
	return FALSE;
}
...
xb_tool_export(XbToolPrivate *priv, gchar **values, GError **error)
{
	...
	for (guint i = 0; values[i] != NULL; i++) {
		...
		str = xb_silo_export(
			...
			XB_NODE_EXPORT_FLAG_FORMAT_INDENT | XB_NODE_EXPORT_FLAG_INCLUDE_SIBLINGS,
		    error);
		if (str == NULL)
			return FALSE;
		g_print("%s", str);
...
main(int argc, char *argv[])
{
	...
	const GOptionEntry options[] = {{"verbose",
					...
					{"wait",
					 ...
					 G_OPTION_ARG_NONE,
					 &priv->wait,
					 "Return only when the silo is no longer valid",
					 NULL},
					{"profile",
					 'p',
					 0,
					...
					{"tokenize",
					 ...
					 &priv->tokenize,
					 "Tokenize elements for faster search, e.g. name,summary",
					 NULL},
					{NULL}};
	...
	g_signal_connect(priv->cancellable, "cancelled", G_CALLBACK(xb_tool_cancelled_cb), priv);
#ifdef HAVE_GIO_UNIX
	g_unix_signal_add_full(G_PRIORITY_DEFAULT, SIGINT, xb_tool_sigint_cb, priv, NULL);
#endif

	/* sort by command name */
	g_ptr_array_sort(priv->cmd_array, (GCompareFunc)xb_tool_sort_command_name_cb);
